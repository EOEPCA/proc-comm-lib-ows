...
void printResults(const std::unique_ptr<EOEPCA::OWS::Param>& input) {
  ...
  if (input->getType() == "BoundingBoxData") {
    auto data = dynamic_cast<EOEPCA::OWS::BoundingBoxData*>(input.get());

  } else if (input->getType() == "ComplexData") {
    auto data = dynamic_cast<EOEPCA::OWS::ComplexData*>(input.get());

    std::cout << "getMaximumMegabytes: " << data->getMaximumMegabytes() << "\n";
  ...
  if (input->getType() == "LiteralData") {
    ...
    if ("string_parameter" == data->getIdentifier()) {
      ...
      EXPECT_EQ(data->getMinOccurs(), 1);
      EXPECT_EQ(data->getMaxOccurs(), 1);
      EXPECT_EQ(data->getDefaultValue(), "default string");
    }

    if ("string_parameter_arr" == data->getIdentifier()) {
      EXPECT_EQ(data->getTitle(), "Title of string_parameter_arr");
      EXPECT_EQ(data->getAbstract(),
                "Abstract describing string_parameter_arr");
      EXPECT_EQ(data->getDataType(), "string");
      EXPECT_EQ(data->getMinOccurs(), 1);
    ...
    std::cout << "getAllowedValuesSize: " << data->getAllowedValues().size()
              << "\n";
...
TEST(eoepcaows_02, inputs_list_from_catalog) {
  ...
  getFromWeb(
      fileBuffer,
      "https://catalog.terradue.com/eoepca-services/search?uid=test_entites");
  ...
  std::unique_ptr<EOEPCA::OWS::OWSContext,
      ...
      ptrContext(lib->parseFromMemory(fileBuffer.c_str(), fileBuffer.size()),
...
TEST(eoepcaows_02, inputs_list_from_catalog_ref) {
  ...
  std::cout << "start download OWS..."
            << "\n";
  std::string fileBuffer{""};
  getFromWeb(fileBuffer,
             "https://catalog.terradue.com/eoepca-services/"
             "search?uid=test_entites_ref");
...
TEST(eoepcaows_02, inputs_enum_optional) {
  auto util = std::make_unique<Util>();
  std::string path{util->getLibName()};
  ...
  std::cout << "start download OWS..."
            << "\n";
  std::string fileBuffer{""};
  // getFromWeb(fileBuffer,"https://raw.githubusercontent.com/bbrauzzi/blasCool/master/snapping-evo-stack-creation.0.1.0.develop.cwl");
  getFromWeb(
      fileBuffer,
      "https://raw.githubusercontent.com/EOEPCA/proc-comm-lib-ows/feature/"
      "input_enum_fix/tests/application/assets/test_entites_ref_2.yaml");
  ...
  std::string applicationFile = fileBuffer.c_str();
  std::string head =
      R"(<?xml version="1.0" encoding="utf-8"?><feed xmlns="http://www.w3.org/2005/Atom"><entry><owc:offering xmlns:owc="http://www.opengis.net/owc/1.0" code="http://www.opengis.net/eoc/applicationContext/cwl"><owc:content type="application/cwl"><![CDATA[)";
  std::string tail = R"(]]></owc:content></owc:offering></entry></feed>)";
  applicationFile = head;
  applicationFile.append(fileBuffer);
